...
export const $ = (selector: string, target?: string): HTMLElement => {
  if (target !== undefined) return $(target).querySelector(selector) as HTMLElement;
...
export const $closest = (current: HTMLElement, selector: string) => {
  const $parent = current.closest(selector);
  if (!($parent instanceof HTMLElement)) throw new Error(`Not Defined ${selector}`);
  return $parent;
};
 ...
 * @returns
 */
export const deepClone = (state: object) => {
...
export const entries = <TYPE>(obj: TYPE): Entries<TYPE> => Object.entries(obj) as any;
