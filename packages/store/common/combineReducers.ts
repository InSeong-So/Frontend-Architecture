import { AnyState, Action, Reducer } from 'store';

/**
 * 객체 내의 모든 키-값 쌍에 함수를 적용합니다.
 */
const setCallback = (target: AnyState, callback: any) => {
  return Object.keys(target).reduce((result: AnyState, key: string) => {
...
export const combineReducers = (reducers: AnyState) => {
  ...
  return (state = defaultState, action: Action) => {
    const callback = (reducer: Reducer, key: string) => {
